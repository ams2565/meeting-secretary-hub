...
export default function SqlQueriesPage() {
  ...
  async function handleSend() {
    ...
    try {
      const result = await streamFetch("/api/sql-queries", { messages: nextMessages }, (acc) => {
    ...
    <div className="flex flex-1 flex-col bg-neutral-950">
      <header className="border-b border-neutral-800 px-6 py-8 sm:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-1 text-neutral-400">
            테이블 구조와 원하는 조회 내용을 이야기해주세요. 대화하면서 SQL 쿼리를 함께 완성해갑니다. (실행은 직접 하셔야 합니다)
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-6 py-8 sm:px-10">
        ...
        <ChatPanel
          ...
          onSend={handleSend}
          loading={loading}
          placeholder="예: users(id, name, email), orders(id, user_id, amount) — 최근 30일간 10만원 이상 주문한 사용자 조회"
