...
export default function ContractReviewPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await streamFetch("/api/contract-review", { contract }, setResult);
    ...
    <div className="flex flex-1 flex-col bg-neutral-950">
      <header className="border-b border-neutral-800 px-6 py-8 sm:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-2 text-2xl font-bold tracking-tight">Contract Review</h1>
          <p className="mt-1 text-neutral-400">
            계약서를 붙여넣으면 위험 조항을 찾아 수정안을 제안합니다. (변호사 검토 대체 아님)
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-8 sm:px-10">
        ...
        {result && (
          <ResultPanel title={loading ? "실시간 검토 중..." : "검토 결과"} content={result} />
        )}
      </main>
    </div>
  );
