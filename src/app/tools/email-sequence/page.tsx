...
export default function EmailSequencePage() {
  ...
  async function handleSend() {
    ...
    try {
      const result = await streamFetch("/api/email-sequence", { messages: nextMessages }, (acc) => {
    ...
    <div className="flex flex-1 flex-col bg-neutral-950">
      <header className="border-b border-neutral-800 px-6 py-8 sm:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-1 text-neutral-400">
            시퀀스 목적을 이야기해주세요. 대화하면서 자동 발송 이메일 시리즈를 함께 완성해갑니다.
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-6 py-8 sm:px-10">
        ...
        <ChatPanel
          ...
          onSend={handleSend}
          loading={loading}
          placeholder="예: 무료 체험 가입자를 유료 전환시키는 온보딩 시퀀스, 회의록 자동 요약 SaaS"
