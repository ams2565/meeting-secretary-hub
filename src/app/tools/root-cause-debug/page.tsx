...
export default function RootCauseDebugPage() {
  ...
  async function handleSend() {
    ...
    try {
      const result = await streamFetch("/api/root-cause-debug", { messages: nextMessages }, (acc) => {
    ...
    <div className="flex flex-1 flex-col bg-neutral-950">
      <header className="border-b border-neutral-800 px-6 py-8 sm:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-1 text-neutral-400">
            에러 메시지와 상황을 이야기해주세요. 필요하면 먼저 질문드리고, 근본 원인을 함께 찾아갑니다.
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-6 py-8 sm:px-10">
        ...
        <ChatPanel
          ...
          onSend={handleSend}
          loading={loading}
          placeholder="예: TypeError: Cannot read properties of undefined (reading 'map') — 관련 코드도 붙여넣으면 더 정확해집니다"
