...
export default function DcfModelPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await streamFetch("/api/dcf-model", { business, assumptions }, setResult);
    ...
    <div className="flex flex-1 flex-col bg-neutral-950">
      <header className="border-b border-neutral-800 px-6 py-8 sm:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-2 text-2xl font-bold tracking-tight">DCF Model</h1>
          <p className="mt-1 text-neutral-400">
            사업 개요를 입력하면 미래 현금흐름 할인법으로 기업가치를 추정합니다.
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-8 sm:px-10">
        ...
        {result && (
          <ResultPanel title={loading ? "실시간 계산 중..." : "DCF 기업가치 추정"} content={result} />
        )}
      </main>
    </div>
  );
